...
/* Busy-loop iters times. The empty asm keeps the loop from being optimised away while
   leaving the counter in a register, so the cost is purely CPU cycles -- the same
   wherever it runs. Calibration and the jobs match with no task-matching memory. */
...
static sys_exit_e job0() { do_work(g_iters[0]); g_done[0]++; return SYS_OK; }
static sys_exit_e job1() { do_work(g_iters[1]); g_done[1]++; return SYS_OK; }
static sys_exit_e job2() { do_work(g_iters[2]); g_done[2]++; return SYS_OK; }
static sys_exit_e (*const JOBS[NTASKS])(void) = { job0, job1, job2 };
...
void edf_run(void)
{
    ...
    telemetry_init();
    telemetry_name(SM_SCHED_ALL, "sched_all");
    telemetry_name(SM_DISPATCH,  "dispatch");
    telemetry_name(SM_IDLE,      "idle");
    uint32_t ro;
    pmu_calibrate(&ro);
    g_telemetry.read_overhead = ro;
    ...
    /* cycles per calibration burst -- do_work is memory-independent, so no heap word. */
    ...
    metric_reset(&g_telemetry.metric[SM_SCHED_ALL]);
    metric_reset(&g_telemetry.metric[SM_DISPATCH]);
    metric_reset(&g_telemetry.metric[SM_IDLE]);
    g_running = 1;
    __asm__ __volatile__("cpsie i" ::: "memory");
