/* Bare-metal port of drivers/ddr/altera/sequencer.c.
 * Framework dependencies replaced by seq_compat.h.
 * Config data taken from boot/sdram_config.h (DE1-SoC QTS values).
 */
#include "seq_compat.h"
#include "sdram_config.h"
#include "sequencer.h"

/* ---- config accessor functions (ported from wrap_sdram_config.c) ---- */

/* Field bit offsets (from sdram_gen5.h) */
#define CTRLCFG_MEMTYPE_LSB     0
#define CTRLCFG_MEMBL_LSB       3
#define CTRLCFG_ADDRORDER_LSB   8
#define CTRLCFG_ECCEN_LSB       10
#define CTRLCFG_ECCCORREN_LSB   11
#define CTRLCFG_REORDEREN_LSB   15
#define CTRLCFG_STARVELIMIT_LSB 16
#define CTRLCFG_DQSTRKEN_LSB    22
#define CTRLCFG_NODMPINS_LSB    23

#define TIMING1_TCWL_LSB  0
#define TIMING1_TAL_LSB   4
#define TIMING1_TCL_LSB   9
#define TIMING1_TRRD_LSB  14
#define TIMING1_TFAW_LSB  18
#define TIMING1_TRFC_LSB  24

#define TIMING2_TREFI_LSB 0
#define TIMING2_TRCD_LSB  13
#define TIMING2_TRP_LSB   17
#define TIMING2_TWR_LSB   21
#define TIMING2_TWTR_LSB  25

#define TIMING3_TRTP_LSB  0
#define TIMING3_TRAS_LSB  4
#define TIMING3_TRC_LSB   9
#define TIMING3_TMRD_LSB  15
#define TIMING3_TCCD_LSB  19

#define TIMING4_SELFRFSHEXIT_LSB 0
#define TIMING4_PWRDOWNEXIT_LSB  10

#define LOWPWR_AUTOPDCYCLES_LSB      0
#define LOWPWR_CLKDISABLECYCLES_LSB  16

#define DRAMODT_WRITE_LSB 0
#define DRAMODT_READ_LSB  4

#define EXTRATIME1_RD_TO_WR_LSB       20
#define EXTRATIME1_RD_TO_WR_BC_LSB    24
#define EXTRATIME1_RD_TO_WR_DIFF_LSB  28

#define DRAMADDRW_COLBITS_LSB  0
#define DRAMADDRW_ROWBITS_LSB  5
#define DRAMADDRW_BANKBITS_LSB 10
#define DRAMADDRW_CSBITS_LSB   13

#define LOWPWREQ_SELFRFSHMASK_LSB 4
#define STATICCFG_MEMBL_LSB       0
#define STATICCFG_USEECCASDATA_LSB 2
#define CTRLWIDTH_LSB             0
#define PORTCFG_AUTOPCHEN_LSB     10
#define FIFOCFG_SYNCMODE_LSB      0
#define FIFOCFG_INCSYNC_LSB       10
#define MPPRIORITY_LSB            0
#define MPWEIGHT1_STATIC_LSB      0
#define MPWEIGHT1_SUM_LSB         18
#define MPPACING1_THRESH1_LSB     0
#define MPPACING1_THRESH2_LSB     28

/* errata: ROWBITS = log2(4GB / cs=1 / bus_bytes=1 / banks=8 / cols=1024) = 19 */
#define SDR_ERRATA_ROWS  19U

static const struct socfpga_sdram_config sdram_cfg = {
    .ctrl_cfg =
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_MEMTYPE    << CTRLCFG_MEMTYPE_LSB)  |
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_MEMBL      << CTRLCFG_MEMBL_LSB)    |
        (0U                                            << CTRLCFG_ADDRORDER_LSB)|
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_ECCEN      << CTRLCFG_ECCEN_LSB)    |
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_ECCCORREN  << CTRLCFG_ECCCORREN_LSB)|
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_REORDEREN  << CTRLCFG_REORDEREN_LSB)|
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_STARVELIMIT << CTRLCFG_STARVELIMIT_LSB)|
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_DQSTRKEN   << CTRLCFG_DQSTRKEN_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_CTRLCFG_NODMPINS   << CTRLCFG_NODMPINS_LSB),
    .dram_timing1 =
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING1_TCWL << TIMING1_TCWL_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING1_AL   << TIMING1_TAL_LSB)  |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING1_TCL  << TIMING1_TCL_LSB)  |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING1_TRRD << TIMING1_TRRD_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING1_TFAW << TIMING1_TFAW_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING1_TRFC << TIMING1_TRFC_LSB),
    .dram_timing2 =
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING2_IF_TREFI << TIMING2_TREFI_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING2_IF_TRCD  << TIMING2_TRCD_LSB)  |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING2_IF_TRP   << TIMING2_TRP_LSB)   |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING2_IF_TWR   << TIMING2_TWR_LSB)   |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING2_IF_TWTR  << TIMING2_TWTR_LSB),
    .dram_timing3 =
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING3_TRTP << TIMING3_TRTP_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING3_TRAS << TIMING3_TRAS_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING3_TRC  << TIMING3_TRC_LSB)  |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING3_TMRD << TIMING3_TMRD_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING3_TCCD << TIMING3_TCCD_LSB),
    .dram_timing4 =
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING4_SELFRFSHEXIT << TIMING4_SELFRFSHEXIT_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMTIMING4_PWRDOWNEXIT  << TIMING4_PWRDOWNEXIT_LSB),
    .lowpwr_timing =
        ((u32)CFG_HPS_SDR_CTRLCFG_LOWPWRTIMING_AUTOPDCYCLES     << LOWPWR_AUTOPDCYCLES_LSB)     |
        ((u32)CFG_HPS_SDR_CTRLCFG_LOWPWRTIMING_CLKDISABLECYCLES << LOWPWR_CLKDISABLECYCLES_LSB),
    .dram_odt =
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMODT_WRITE << DRAMODT_WRITE_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMODT_READ  << DRAMODT_READ_LSB),
    .extratime1 =
        ((u32)CFG_HPS_SDR_CTRLCFG_EXTRATIME1_CFG_EXTRA_CTL_CLK_RD_TO_WR           << EXTRATIME1_RD_TO_WR_LSB)      |
        ((u32)CFG_HPS_SDR_CTRLCFG_EXTRATIME1_CFG_EXTRA_CTL_CLK_RD_TO_WR_BC        << EXTRATIME1_RD_TO_WR_BC_LSB)   |
        ((u32)CFG_HPS_SDR_CTRLCFG_EXTRATIME1_CFG_EXTRA_CTL_CLK_RD_TO_WR_DIFF_CHIP << EXTRATIME1_RD_TO_WR_DIFF_LSB),
    .dram_addrw =
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMADDRW_COLBITS                << DRAMADDRW_COLBITS_LSB)  |
        (SDR_ERRATA_ROWS                                             << DRAMADDRW_ROWBITS_LSB)  |
        ((u32)CFG_HPS_SDR_CTRLCFG_DRAMADDRW_BANKBITS                << DRAMADDRW_BANKBITS_LSB) |
        ((u32)(CFG_HPS_SDR_CTRLCFG_DRAMADDRW_CSBITS - 1U)           << DRAMADDRW_CSBITS_LSB),
    .dram_if_width  = CFG_HPS_SDR_CTRLCFG_DRAMIFWIDTH_IFWIDTH,
    .dram_dev_width = CFG_HPS_SDR_CTRLCFG_DRAMDEVWIDTH_DEVWIDTH,
    .dram_intr      = CFG_HPS_SDR_CTRLCFG_DRAMINTR_INTREN,
    .lowpwr_eq      = ((u32)CFG_HPS_SDR_CTRLCFG_LOWPWREQ_SELFRFSHMASK << LOWPWREQ_SELFRFSHMASK_LSB),
    .static_cfg     =
        ((u32)CFG_HPS_SDR_CTRLCFG_STATICCFG_MEMBL        << STATICCFG_MEMBL_LSB)       |
        ((u32)CFG_HPS_SDR_CTRLCFG_STATICCFG_USEECCASDATA << STATICCFG_USEECCASDATA_LSB),
    .ctrl_width  = CFG_HPS_SDR_CTRLCFG_CTRLWIDTH_CTRLWIDTH,
    .cport_width = CFG_HPS_SDR_CTRLCFG_CPORTWIDTH_CPORTWIDTH,
    .cport_wmap  = CFG_HPS_SDR_CTRLCFG_CPORTWMAP_CPORTWMAP,
    .cport_rmap  = CFG_HPS_SDR_CTRLCFG_CPORTRMAP_CPORTRMAP,
    .rfifo_cmap  = CFG_HPS_SDR_CTRLCFG_RFIFOCMAP_RFIFOCMAP,
    .wfifo_cmap  = CFG_HPS_SDR_CTRLCFG_WFIFOCMAP_WFIFOCMAP,
    .cport_rdwr  = CFG_HPS_SDR_CTRLCFG_CPORTRDWR_CPORTRDWR,
    .port_cfg    = ((u32)CFG_HPS_SDR_CTRLCFG_PORTCFG_AUTOPCHEN << PORTCFG_AUTOPCHEN_LSB),
    .fpgaport_rst = CFG_HPS_SDR_CTRLCFG_FPGAPORTRST,
    .fifo_cfg    =
        ((u32)CFG_HPS_SDR_CTRLCFG_FIFOCFG_SYNCMODE << FIFOCFG_SYNCMODE_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_FIFOCFG_INCSYNC  << FIFOCFG_INCSYNC_LSB),
    .mp_priority  = CFG_HPS_SDR_CTRLCFG_MPPRIORITY_USERPRIORITY,
    .mp_weight0   = CFG_HPS_SDR_CTRLCFG_MPWIEIGHT_0_STATICWEIGHT_31_0,
    .mp_weight1   =
        ((u32)CFG_HPS_SDR_CTRLCFG_MPWIEIGHT_1_STATICWEIGHT_49_32 << MPWEIGHT1_STATIC_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_MPWIEIGHT_1_SUMOFWEIGHT_13_0   << MPWEIGHT1_SUM_LSB),
    .mp_weight2   = CFG_HPS_SDR_CTRLCFG_MPWIEIGHT_2_SUMOFWEIGHT_45_14,
    .mp_weight3   = CFG_HPS_SDR_CTRLCFG_MPWIEIGHT_3_SUMOFWEIGHT_63_46,
    .mp_pacing0   = CFG_HPS_SDR_CTRLCFG_MPPACING_0_THRESHOLD1_31_0,
    .mp_pacing1   =
        ((u32)CFG_HPS_SDR_CTRLCFG_MPPACING_1_THRESHOLD1_59_32 << MPPACING1_THRESH1_LSB) |
        ((u32)CFG_HPS_SDR_CTRLCFG_MPPACING_1_THRESHOLD2_3_0   << MPPACING1_THRESH2_LSB),
    .mp_pacing2   = CFG_HPS_SDR_CTRLCFG_MPPACING_2_THRESHOLD2_35_4,
    .mp_pacing3   = CFG_HPS_SDR_CTRLCFG_MPPACING_3_THRESHOLD2_59_36,
    .mp_threshold0 = CFG_HPS_SDR_CTRLCFG_MPTHRESHOLDRST_0_THRESHOLDRSTCYCLES_31_0,
    .mp_threshold1 = CFG_HPS_SDR_CTRLCFG_MPTHRESHOLDRST_1_THRESHOLDRSTCYCLES_63_32,
    .mp_threshold2 = CFG_HPS_SDR_CTRLCFG_MPTHRESHOLDRST_2_THRESHOLDRSTCYCLES_79_64,
    .phy_ctrl0    = CFG_HPS_SDR_CTRLCFG_PHYCTRL_PHYCTRL_0,
};

static const struct socfpga_sdram_rw_mgr_config rw_mgr_cfg = {
...
static const struct socfpga_sdram_io_config io_cfg = {
...
static const struct socfpga_sdram_misc_config misc_cfg = {
...
const struct socfpga_sdram_config *socfpga_get_sdram_config(void) { return &sdram_cfg; }
const struct socfpga_sdram_rw_mgr_config *socfpga_get_sdram_rwmgr_config(void) { return &rw_mgr_cfg; }
const struct socfpga_sdram_io_config *socfpga_get_sdram_io_config(void) { return &io_cfg; }
const struct socfpga_sdram_misc_config *socfpga_get_sdram_misc_config(void) { return &misc_cfg; }

void socfpga_get_seq_ac_init(const u32 **init, unsigned int *nelem)
{
    *init = ac_rom_init;
    *nelem = sizeof(ac_rom_init) / sizeof(ac_rom_init[0]);
}

void socfpga_get_seq_inst_init(const u32 **init, unsigned int *nelem)
{
    *init = inst_rom_init;
    *nelem = sizeof(inst_rom_init) / sizeof(inst_rom_init[0]);
}

/* ---- original sequencer logic below (includes replaced) ---- */
